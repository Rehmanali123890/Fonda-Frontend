...
@Component({
  selector: 'app-grubhub-details',
  templateUrl: './grubhub-details.component.html',
  styleUrls: ['./grubhub-details.component.css']
})
export class GrubhubDetailsComponent {
  startDate: string
  endDate: string
  source: string
  type: string
  ...
  getfinancereport() {
    ...
    this.merchantService.getfinancepayout(this.merchantId, mData).subscribe((data: any) => {
      ...
      if (this.financepayoutcalculationObj && this.financepayoutcalculationObj['transactions']) {
        try {
            ...
            this.financepayoutcalculationObj['transactions'] = []; // Fallback to an empty array
        }
    } else {
        this.financepayoutcalculationObj['transactions'] = []; // Fallback for undefined or null
    }
  ...
  downloadTransactionReport() {
    ...
    const platform = "grubhub"; // Use the platform you're interested in
  ...
  downloadOriginalFile(format: string) {
    ...
    const fileName = `Grubhub Transaction Summary Report - ${this.selectedMerchant} (from ${startDate} to ${endDate})`;
