...
@UntilDestroy()
@Component({
  selector: 'app-doordash',
  templateUrl: './doordash.component.html',
  styleUrls: ['./doordash.component.scss']
})
export class DoordashComponent implements OnChanges, OnInit {
  ...
  triggerManualSync(platformType: number) {
    this.merchantService.TriggerManualSync(this.merchantId, 'f3f2613a-5340-4e55-b3b8-4f19a323351f').subscribe((data: any) => {
      ...
      if (loopCount-- < 0) {
        ...
        if (href != null) {
          ...
          if (href.match('code')) {
            ...
            this.windowHandle.close();
            if (this.connectDoordash == true) {
              this.connectDoordash = false
              this.merchantService.ConnectUbereats({
                "code": this.authorizationCode,
                "redirect_uri": window.location.origin
              }, this.merchantId).subscribe((data: any) => {
                // this.GetConnectedPlatforms()
                this.toaster.success("Doordash Connected");
                this.triggerChangeParent.emit();
              },
                (err) => {
                  console.log(err)
