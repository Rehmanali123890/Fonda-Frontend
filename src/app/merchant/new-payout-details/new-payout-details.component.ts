...
@Component({
  selector: 'app-new-payout-details',
  templateUrl: './new-payout-details.component.html',
  styleUrls: ['./new-payout-details.component.css']
})
export class NewPayoutDetailsComponent implements OnInit {
  ...
  reCalculatePayout(status) {
    ...
    if (status == 1) {
      // this.netPayout = this.localNetPayoutBackup;
      this.netPayout = this.netPayout - this.payoutAdjustmentsBackup
      if (this.payoutAdjustments !== undefined || this.payoutAdjustments != null) {
        this.netPayout = this.netPayout + this.payoutAdjustments
      ...
      if (this.marketingFee !== undefined || this.marketingFee != null) {
        ...
        this.netPayout = this.netPayout - this.marketingFeeBackup
        this.netPayout = this.netPayout + this.marketingFee
  ...
  getTransferList() {
    ...
    this.merchantService.GetFinanceMerchantTransfers(this.merchantId).subscribe((data: any) => {
  ...
  getBankAccountDetails(payoutid, status?: number, mData?, transferToBank?) {
    this.checkBankDetail = false
    this.gettingTransfertoBank = true
    this.merchantService.getBankDetailsFinancePayout(this.merchantId, payoutid).subscribe((data: any) => {
      console.log("Response for bank account detail", data)
      if (data.length == 0) {
        this.checkBankDetail = true
        this.transferToBank(payoutid, status, mData, transferToBank)
      }
      else {
  ...
  calculatePayout(status, swarltitle?: string) {
    ...
    if (status == 0) {
      this.merchantTreasuryService.getAccountDetails(this.merchantId).subscribe((data: any) => {
        ...
        console.log(" this.TreasuryBalance ", this.TreasuryBalance)
        if (data.hasOwnProperty('message')) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            if (this.transferType == 2) {
              this.getBankAccountDetails('', status, mData, true)
            }
            else {
              this.calculateOrTransfer(status, mData, true)
            }
          } else {
            this.gettingTransfers = false;
  ...
  calculateOrTransfer(status, mData, transferToBank?) {
    this.merchantService.getfinancepayout(this.merchantId, mData).subscribe((data: any) => {

      this.financepayoutcalculationObj = data.data
      console.log('data.data treausry resp ', data)
      if (this.financepayoutcalculationObj) {
        console.log(this.financepayoutcalculationObj['payout_adjustment'])
        this.netPayout = this.financepayoutcalculationObj['netPayout']
        this.remarks = this.financepayoutcalculationObj['remarks']
        this.payoutAdjustments = this.financepayoutcalculationObj['payout_adjustment']
        if (this.financepayoutcalculationObj['payout_adjustment']) {
          this.netPayout = this.financepayoutcalculationObj['netPayout'] + this.financepayoutcalculationObj['payout_adjustment']
          this.payoutAdjustmentsBackup = this.financepayoutcalculationObj['payout_adjustment']
        }
  ...
  financeReportSummaryNewFormat() {
    ...
    this.merchantService.sendSummaryEmailFinancepayout(this.merchantId, {
      'startDate': this.startDate,
      'endDate': this.endDate,
      "payoutType": this.payoutType,
  ...
  revertTransfer(id) {
    ...
    this.merchantService.ReverMerchantAmountTransferFinance(this.merchantId, id).subscribe((data: any) => {
      this.showDetails = false;
  ...
  transferToBank(id, status?: number, mData?, transferToBank?) {
    var title = ''
    var confirmButtonText = ''
    ...
    if (this.checkBankDetail == false) {
      if (this.accountDetail.last4 == this.accountDetail.merchantlast4) {
        this.translocoService.selectTranslate('reuseableWords.Are you sure?').subscribe(translation => {
          title = translation;
        });
        ...
        this.translocoService.selectTranslate('reuseableWords.Account Detail Mismatch').subscribe(translation => {
          title = translation;
        });
        this.translocoService.selectTranslate('reuseableWords.accountmismatch').subscribe(translation => {
          text = translation;
        });
      }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (this.transferType == 2 && transferToBank) {
          this.calculateOrTransfer(status, mData, true)
        }
        else {
          this.transferringBank = true;
          this.merchantService.transferToBankFinancePayout(id, this.merchantId).subscribe((data: any) => {
  ...
  getHtmlContent(text, status) {
    if (status == 0) {
      ...
      ${this.accountDetail.last4 == this.accountDetail.merchantlast4 // Check your condition here
          ? ""
          : `<div style="display: flex; align-items: baseline; margin-bottom: 5px;">
              <span style="width: 200px; font-weight: bold; text-align: right; padding-right: 4px;">Fonda Account Number:</span>
              <span>********${this.accountDetail.merchantlast4}</span>
            </div>`
        }
    </div>
    `;
