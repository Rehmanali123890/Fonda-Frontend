...
@UntilDestroy()
@Component({
  selector: 'app-problematic-transaction-report',
  templateUrl: './problematic-transaction-report.component.html',
  styleUrls: ['./problematic-transaction-report.component.css']
})

export class ProblematicTransactionReportComponent {
  constructor( private merchantService: MerchantService, private route: ActivatedRoute, private appState: AppStateService, private securityService: SecurityService ,private toaster: ToastrService,private translocoService: TranslocoService,private activatedRoute: ActivatedRoute) { }
    ...
    filterType = 2;
    emailBTN = 1;
    platform_type = 'Doordash';
    platform_type_value = 1;
        ...
        .subscribe(
          (data: MerchantListDto[]) => {
            ...
            if (this.MerchantsList && this.MerchantsList.length) {
              ...
              this.generateNegativeNoOrderReport();
            }
    ...
    selectPlatformType() {
      if (this.platform_type_value == 1) {
        this.platform_type = 'Doordash';
      } else if (this.platform_type_value == 2) {
        this.platform_type = 'UberEats';
      } else if (this.platform_type_value == 3) {
        this.platform_type = 'Grubhub';
      } else if (this.platform_type_value == 4) {
        this.platform_type = 'Storefront';
      } else if (this.platform_type_value == 5) {
        this.platform_type = 'All';
      }
      console.log("platform type", this.platform_type)
      console.log("platform type value", this.platform_type_value)
    }
    ...
    getModifiedHeaders() {
      return this.headers; // Slices off the first two columns (payout_id, payout_type)
    }
    getModifiedRows() {
      return this.dataRows; // Slices off the first two columns (payout_id, payout_type)
    }
    formatAndEncodeDate(date: string): string {
      const formattedDate = new Date(date).toISOString().split('T')[0]; // Formats to 'yyyy-MM-dd'
      return encodeURIComponent(formattedDate); // Encodes the date
    }


    generateNegativeNoOrderReport(directDownload: boolean = false): Promise<any> {
      return new Promise((resolve, reject) => {
        ...
        const PlatformType = this.platform_type;
        console.log("---------------")
        this.merchantService.getNegativeAndNoOrderReport(this.merchantId, startDate, endDate, PlatformType).subscribe(
          
          (response: Blob) => {
            const reader = new FileReader();
            reader.onload = () => {
              const data = new Uint8Array(reader.result as ArrayBuffer);
              console.log("Data of negative records---->", data)
              const workbook = XLSX.read(data, { type: 'array' });
              console.log("workbook of negative records---->", workbook)
              const sheet = workbook.Sheets[workbook.SheetNames[0]];
              console.log("sheet of negative records---->", sheet)
              const jsonData: any[] = XLSX.utils.sheet_to_json(sheet, { header: 1 });
              console.log("json data of negative records---->", jsonData)
              ...
              if (!directDownload) {
                if (jsonData.length > 1) {
                  ...
                  this.headers = ["No records available for the given period."];
                  this.dataRows = []; 
                }
          ...
          (err) => {
            ...
            reader.onload = () => {
              try {
                ...
                this.toaster.error(errorJson.message || "An unexpected error occurred.");
                console.error("Backend error:", errorJson);
              } catch (parseError) {
                this.toaster.error("An unexpected error occurred.");
                console.error("Error parsing backend error response:", reader.result);
              }
              this.parsingData = false;
              reject(err);
            };
            reader.readAsText(err.error); // Read error Blob as text
          }
        );
      });
    }
    ...
    downloadProblematicFile(format: string) {
      this.exporting = true
      const startDate = new Date(this.startDate).toISOString().split('T')[0];
      const endDate = new Date(this.endDate).toISOString().split('T')[0];
      this.generateNegativeNoOrderReport(true)
        .then((jsonData) => {
          if (jsonData.length <= 1) {
            var headers = "No records available for the given period.";
          }
          ...
          console.log("Json data for file download -->",jsonData)
          ...
          const fileName = `Problematic Transaction Report (from ${startDate} to ${endDate})`;
